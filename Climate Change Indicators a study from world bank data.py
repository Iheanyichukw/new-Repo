# -*- coding: utf-8 -*-
"""
Created on Tue Apr  4 13:49:52 2023
...
df = pd.read_csv('API_19_DS2_en_csv_v2_5361599.csv')

dfc = pd.read_csv('Metadata_Country_API_19_DS2_en_csv_v2_5361599.csv')
...
df['mean'] = df[['2010', '2011', '2012', '2013', '2014', '2015', '2016',
                 '2017', '2018', '2019', '2020', '2021']].mean(axis=1)


selected_indicators = ['Urban population (% of total population)', 'Population growth (annual %)',
                       'Mortality rate, under-5 (per 1,000 live births)', 'CO2 emissions (kt)',
                       'Electric power consumption (kWh per capita)', 'Energy use (kg of oil equivalent) per $1,000 GDP (constant 2017 PPP)',
                       'Access to electricity (% of population)', 'Arable land (% of land area)']

countries = ['Australia', 'China', 'United Arab Emirates',
             'Germany', 'Nigeria', 'United States', 'Pakistan']
...
for ind in selected_indicators:
    ...
    for i in df_year.columns:
        sns.swarmplot(y='Country Name', x=i,
                      data=df_year.loc[countries, :].reset_index())
...
for i in selected_indicators:
    ...
    temp_df.groupby(['Region'])['mean'].mean().sort_values(
...
n_df.sort_values(by='arable land', ascending=False)[
    :10].plot(kind='bar', figsize=(4, 3))
plt.ylabel('frequency')
plt.title('Top 10 Countries with Max Arable Land')
plt.show()

n_df.sort_values(by='forest land', ascending=False)[
    :10].plot(kind='bar', figsize=(4, 3))
plt.ylabel('frequency')
...
df1_mean = df1.mean(axis=1).reset_index().rename(
    {0: 'Access to Electricity'}, axis=1)
df3_mean = df3.mean(axis=1).reset_index().rename(
    {0: 'Electric Power Consumption'}, axis=1)

n_df = df1_mean.merge(df3_mean, on='Country Name')

plt.figure(figsize=(4, 3))
sns.scatterplot(x=n_df['Access to Electricity'],
                y=n_df['Electric Power Consumption'])
...
n_df.index = n_df.loc[:, 'Country Name']
n_df.sort_values(by='Access to Electricity', ascending=False)[
    :10].plot(kind='bar', figsize=(4, 3))
plt.ylabel('frequency')
plt.title('Top 10 Countries with Max Access to Electricity')
plt.show()

n_df.sort_values(by='Electric Power Consumption', ascending=False)[
    :10].plot(kind='bar', figsize=(4, 3))
plt.ylabel('frequency')
...
df1, df2 = data_ingestion(
    df, 'Energy use (kg of oil equivalent) per $1,000 GDP (constant 2017 PPP)')
...
sns.heatmap(df1[selected_indicators].corr(),
            cmap='viridis', linewidths=.5, annot=True)
...
plt.figure(figsize=(10, 7))
sns.heatmap(df_country[countries].corr(),
            cmap='viridis', linewidths=.5, annot=True)
...
df_year, df_country = data_ingestion(
